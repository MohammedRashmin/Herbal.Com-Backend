...
public class ProductService : IProductService
{
    ...
    public async Task<ProductDto> AddProductImageAsync(int productId, IFormFile imageFile)
    {
        ...
        if (count >= 5) throw new InvalidOperationException("Maximum 5 images allowed per product");
        ...
        await _productRepository.AddImageAsync(new ProductImage { ProductId = productId, ImageUrl = imageUrl });
    ...
    // User Methods
    public async Task<IEnumerable<ProductListDto>> GetProductsUserAsync(int? categoryId, bool? isFeatured, string? sortBy, int page, int pageSize)
    {
        var products = await _productRepository.GetAllAsync();
        var query = products.Where(p => p.Stock > 0);

        if (categoryId.HasValue) query = query.Where(p => p.CategoryId == categoryId.Value);
        if (isFeatured.HasValue && isFeatured.Value) query = query.Where(p => p.IsFeatured);
        ...
        return query.Skip((page - 1) * pageSize).Take(pageSize).Select(p => new ProductListDto
        {
            Id = p.Id,
            Name = p.Name,
    ...
    public async Task<ProductDetailDto?> GetProductDetailAsync(int id)
    {
        ...
        return new ProductDetailDto
        {
            ...
            Reviews = p.Reviews?.Where(r => r.IsApproved).Select(r => new ReviewDto
            {
                Id = r.Id,
    ...
    public async Task<IEnumerable<ProductListDto>> SearchProductsAsync(string keyword)
    {
        var products = await _productRepository.SearchAsync(keyword, 20);
